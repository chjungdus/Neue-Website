...
function AnimatedNumber({ target, suffix, decimal }: StatData) {
  ...
  return (
    <div
      ref={containerRef}
      className="text-5xl md:text-6xl font-black text-[#111827] tabular-nums leading-none"
    >
      {display}
      {suffix}
    </div>
          ...
          {stats.map((stat, i) => (
            <FadeIn key={stat.label} delay={i * 0.08} direction="up">
              <div className="text-center">
                <AnimatedNumber {...stat} />
                <div className="text-[#111827] font-semibold text-sm mt-3 mb-1">
                  {stat.label}
                </div>
